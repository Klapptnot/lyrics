...
pub struct LyricParagraph {
  pub lines: Vec<String>,
  pub title: String, // Discard type, its always lyrics
}
      ...
      for lp in data {
        me.push(LyricParagraph {
          lines: lp["lines"]
            ...
            .unwrap()
            .iter()
            .map(|l| Value::as_str(l).unwrap().into())
            .collect::<Vec<String>>(),
...
pub(crate) struct TrackInfo {
  ...
  pub has_lyrics_struct: bool,
  /// Lyrics (full) language name
  pub lyrics_lang: String,
  /// The lyrics as string (escaped)
  pub lyrics: String,
  /// A structured lyrics representation
  pub lyrics_struct: Vec<LyricParagraph>,
  /// Lyric composer(s)
  pub lyrics_copyright: String,
  /// Song primary genre
  pub genre: String,
  // Cover image URL (expect a 350x350 px jpg file)
  pub cover: String,
    ...
    if let Some(data) = data {
      return Some(Self {
        ...
        cover: Value::as_str(&data["props"]["pageProps"]["data"]["trackInfo"]["data"]["track"]["coverImage"])
          .unwrap()
          .into(),
        released: Value::as_str(&data["props"]["pageProps"]["data"]["trackInfo"]["data"]["track"]["releaseDate"]).unwrap()
          [0..10]
          .into(),
        spotify: format!(
...
impl MxmAPI {
  ...
  pub fn get_from_url(&self, url: &String) -> TrackInfo {
    ...
    spinner.stop();
    let mxm_json: String = mxm_json.unwrap_or_else(|| macros::exit_err("Couldn't get a (valid) response from the server"));
    let track = TrackInfo::from(mxm_json).unwrap_or_else(|| macros::exit_err("Couldn't read json from response"));
  ...
  pub fn get_from_keywords(&self, keyword: &String, index: usize) -> TrackInfo {
    ...
    let mut urls_list: Option<Vec<(String, String)>> = None;
    for i in 1..self.tries {
    ...
    MxmAPI::get_from_url(&self, &uti.0.to_string())
  }
}

pub fn get_urls(keyword: &String, timeout: u32, headers_map: Option<HeaderMap>) -> Result<Vec<(String, String)>, String> {
  ...
  let mut urls_tp: Vec<(String, String)> = Vec::new();
  ...
  for i in 0..url_list.len() {
    if tr_re.is_match(url_list[i].as_str()).unwrap() {
      ...
      urls_tp.push((uns_url, String::from(&url_desc[i])))
    } else {
      urls_tp.push((String::from(&url_list[i]), String::from(&url_desc[i])))
    }
  }
